...
class GlobalPay(WebService):
    ...
    def _process_transaction(self, json_data: dict):
        ...
        _result_business_logic = self._some_business_logic(transaction)
        if not _result_business_logic: # business logic failed, skip the rest
            self.logger.log_info(self.service_name, '_process_transaction', 'N/A', f'Validation logic failed, skipping rest of the flow...',)
            return
        result_business_logic = self._save_transaction_to_db(transaction)
        if not result_business_logic: # db save failed, skip the rest
            self.logger.log_info(self.service_name, '_process_transaction', 'N/A', f'DB save failed, skipping rest of the flow...',)
