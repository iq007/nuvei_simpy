...
class ResourceDecorator:
    ...
    def _add_exceptions(self, func, ex):
        def wrapper(*args, **kwargs):
            if random.random() <= self.failure_rate:
                self.logger.log_error(self.service_name, func.__name__, 'N/A', f'{ex.type}: {ex.description} ')
    ...
    def _add_latency(self, func):
        def wrapper(*args, **kwargs):
            _latency = random.uniform(self.latency_min, self.latency_max)
            self.logger.log_info(self.resource.service_name, func.__name__, 'N/A', f'Latency: {_latency}')
            yield self.env.timeout(_latency)
            return func(*args, **kwargs)
        return wrapper
    ...
    def decorate(self):
        ...
        for method in self.latency_methods:
            method_name = method['name']
            decorated_method = self._add_latency(getattr(self.resource, method_name))
            setattr(self.resource, method_name, decorated_method)
